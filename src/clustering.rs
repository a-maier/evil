...
use jetty::{anti_kt_f, cambridge_aachen_f, kt_f, PseudoJet, Cluster};
...
#[derive(Copy, Clone, Default, Eq, PartialEq, Ord, PartialOrd, Hash, Debug, Deserialize, Serialize)]
...
pub fn cluster(event: &Event, jet_def: &JetDefinition) -> Vec<PseudoJet> {
    ...
    let out = Vec::from_iter(
        event.out.iter()
            .filter_map(|p| if p.is_parton() || HADRONS.contains(&p.id) {
                Some(p.p)
            } else {
                None
            })
    ...
    match jet_def.algorithm {
        ...
        JetAlgorithm::CambridgeAachen => out.cluster_if(cambridge_aachen_f(r), pt_cut),
            ...
            .show(ctx, |ui| {
                ...
                ui.add_enabled_ui(
                    ...
                    |ui| {
                        ...
                        ui.horizontal(|ui| {
                            changed |= ui.add(
                                DragValue::new(&mut jet_def.min_pt)
                            ).changed();
